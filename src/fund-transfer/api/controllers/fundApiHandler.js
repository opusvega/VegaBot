...
async function apiHandlerForTransferGetPayee(req,res){
	...
	if(result != true){
		...
		if(result.length === 1){
			...
			returnJsonObj.contextOut.push({
				"name":"transfer-init-followup",
		   		"lifespan":0
			});
...
async function apiHandlerForAddPayeeInit(req,res){
	...
	context.insertContextLog(req,"add-payee-get-nickname");
	return res.json(returnJsonObj);
}
...
async function apiHandlerForAddPayeeGetNickname(req,res){
	...
	let result1 = await mysqlFunctions.insertPayee(req);
	//console.log("Insert paye=====>",result1);
	if (result1 ==  false){
...
async function apiHandlerForAddPayeeGetRoutingnumber(req,res){
	...
	//console.log("AddPayeeGetRoutingnumber routingnumber===>",routingNumber);
	var numberValidate = /^[A-Za-z]+$/;
	if (routingNumber.length != 6 || routingNumber.match(numberValidate)){
		//console.log("AddPayeeGetRoutingnumber inside if loop=====?");
